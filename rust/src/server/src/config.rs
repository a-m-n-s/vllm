...
pub struct ApiServerOptions {
    ...
    /// When `true`, expose the profiler control routes (`/start_profile` and
    /// `/stop_profile`). Mirrors Python, which registers them only when
    /// profiling is enabled via `--profiler-config`.
