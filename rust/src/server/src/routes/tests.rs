...
async fn test_profiler_app_with_engine_script<F>(
    ...
    F: for<'a> FnOnce(&'a mut DealerSocket, &'a mut PushSocket) -> TestFuture<'a> + Send + 'static,
{
    ...
    let chat = ChatLlm::from_shared_backend(test_llm(client), Arc::new(FakeChatBackend::new()));
    let state = AppState::new(vec!["Qwen/Qwen1.5-0.5B-Chat".to_string()], chat)
        .with_api_server_options(ApiServerOptions {
            profiler_enabled,
            ..Default::default()
        });
    (
        build_router_with_dev_mode(Arc::new(state), false),
        engine_task,
    )
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial]
async fn start_profile_route_sends_profile_utility_call() {
    let (app, engine_task) = test_profiler_app_with_engine_script(true, |dealer, push| {
        boxed_test_future(async move {
            let utility = recv_engine_message(dealer).await;
            assert_eq!(utility[0].as_ref(), &[0x03]);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial]
async fn stop_profile_route_sends_profile_utility_call() {
    let (app, engine_task) = test_profiler_app_with_engine_script(true, |dealer, push| {
        boxed_test_future(async move {
            let utility = recv_engine_message(dealer).await;
            assert_eq!(utility[0].as_ref(), &[0x03]);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial]
async fn profile_routes_are_hidden_when_profiler_disabled() {
    // Even with dev mode enabled, the profiler routes stay hidden unless
    // profiling is enabled, mirroring Python's `--profiler-config` gating.
    let (chat, engine_task) = test_chat_with_engine_handle().await;
    let app = build_router_with_dev_mode(
        Arc::new(AppState::new(
            vec!["Qwen/Qwen1.5-0.5B-Chat".to_string()],
    ...
    for uri in ["/start_profile", "/stop_profile"] {
        let response = app
            .clone()
            .call(
                Request::builder()
